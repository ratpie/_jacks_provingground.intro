// by ALIAS
//null=[wildfire,200,120,0.1,-1,false,true] execVM "scripts\AL_fire\al_wild_fire.sqf";
...
if (_spreading>_diameter) then 
{
	...
	[[_source,_diameter,_life_time,_spreading,_craters],"scripts\AL_fire\al_wild_fire_sfx.sqf"] remoteExec ["execvm",0,true];
							...
							if (isNil{_x getVariable "killed_by_fire"}) then 
								{
									...
									[_x,_rnd_lf,_rnd_lf-2] execVM "scripts\AL_fire\al_unit_fire.sqf"
								};
						} 
						else 
							{
	...
	[[_source,_diameter,_life_time,_spreading,_craters],"scripts\AL_fire\al_wild_fire_sfx.sqf"] remoteExec ["execvm",0,true];
	[_source,_diameter,_life_time,_hide_destr_veg] execvm "scripts\AL_fire\al_damage_fire.sqf";
